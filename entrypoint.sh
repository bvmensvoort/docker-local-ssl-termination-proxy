...
sed -i 's/<carddav>//g;s/<\/carddav>//g' /etc/nginx/nginx.conf.template > /etc/nginx/nginx.conf.template;

if [[ -z $CALDAV ]]; then sed -i '/<caldav>/,/<\/caldav>/g' /etc/nginx/nginx.conf.template > /etc/nginx/nginx.conf.template; fi
sed -i 's/<caldav>//g;s/<\/caldav>//g' /etc/nginx/nginx.conf.template > /etc/nginx/nginx.conf.template;
